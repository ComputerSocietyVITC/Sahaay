...
async def update_user(user_id:str, user:UpdateUserModel):
    ...
    user = {k: v for k,v in user.dict().items() if v is not None and k != "password_reverification"}
...
async def get_user_any_field(field_name:str, field_value:str, d_user:Optional[dict]=None):
    if d_user is None:
        if field_value and field_value is not None:
            if (
                user := await db["users"].find({field_name:field_value}).to_list(100)
            ):
                return user
        else:
                user = await db["users"].find().to_list(100)
                return user
    elif not(user := next((item for item in d_user if item[field_name] == field_value), None)):
            return user
